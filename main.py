...
try:
    ...
    print(f"❌ 警告: 无法导入新的HTML生成器，将使用默认模板 - {e}")
except Exception as e:
    USE_NEW_TEMPLATE = False
    print(f"❌ 警告: 导入HTML生成器时出错，将使用默认模板 - {e}")
...
def generate_html_report(
    ...
    if USE_NEW_TEMPLATE:
        # 计算统计数据
        hot_news_count = sum(stat["count"] for stat in stats if stat["count"] > 0)
        word_count = len([stat for stat in stats if stat["count"] > 0])
